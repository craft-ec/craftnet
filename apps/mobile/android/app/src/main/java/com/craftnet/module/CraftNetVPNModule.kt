...
import com.craftnet.vpn.CraftNetVpnService

/**
 * React Native Native Module for CraftNet VPN
 */
class CraftNetVPNModule(reactContext: ReactApplicationContext) :
    ...
    override fun getName(): String = "CraftNetVPN"
    ...
    fun disconnect(promise: Promise) {
        try {
            ...
            val intent = Intent(reactApplicationContext, CraftNetVpnService::class.java).apply {
                action = CraftNetVpnService.ACTION_DISCONNECT
            }
    ...
    fun setPrivacyLevel(level: String, promise: Promise) {
        ...
        val prefs = reactApplicationContext.getSharedPreferences("craftnet", Context.MODE_PRIVATE)
    ...
    fun setMode(mode: String, promise: Promise) {
        ...
        // In production, this would call through JNI to CraftNetUnifiedNode.set_mode()
        promise.resolve(null)
    }
    ...
    fun request(params: ReadableMap, promise: Promise) {
        ...
        try {
            val result = Arguments.createMap().apply {
                putInt("status", 200)
                putString("body", "{\"mock\":true,\"method\":\"$method\",\"url\":\"$url\",\"headers\":$headerCount,\"message\":\"Mock response from CraftNet\"}")
            }
            promise.resolve(result)
        } catch (e: Exception) {
    ...
    private fun startVpnService(config: ReadableMap, promise: Promise) {
        try {
            ...
            val intent = Intent(reactApplicationContext, CraftNetVpnService::class.java).apply {
                action = CraftNetVpnService.ACTION_CONNECT

                if (config.hasKey("privacyLevel")) {
                    putExtra(CraftNetVpnService.EXTRA_PRIVACY_LEVEL, config.getString("privacyLevel"))
                }
                if (config.hasKey("bootstrapPeer")) {
                    putExtra(CraftNetVpnService.EXTRA_BOOTSTRAP_PEER, config.getString("bootstrapPeer"))
                }
            }
