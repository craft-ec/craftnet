...
/// CraftNet Packet Tunnel Provider
///
/// This Network Extension handles VPN tunnel packets by routing them
/// through the CraftNet P2P network via UniFFI Rust bindings.
class PacketTunnelProvider: NEPacketTunnelProvider {

    private let log = OSLog(subsystem: "com.craftnet.vpn", category: "PacketTunnel")
    private var vpnClient: CraftNetUnifiedNode?
    private var isConnected = false
    private let packetQueue = DispatchQueue(label: "com.craftnet.packetQueue", qos: .userInteractive)
    ...
    private let appGroup = "group.com.craftnet.vpn"

    // MARK: - Tunnel Lifecycle

    override func startTunnel(options: [String: NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        os_log("Starting CraftNet tunnel", log: log, type: .info)
        ...
        do {
            // Create unified node
            vpnClient = try CraftNetUnifiedNode(config: config)
    ...
    override func stopTunnel(with reason: NEProviderStopReason, completionHandler: @escaping () -> Void) {
        os_log("Stopping CraftNet tunnel: %{public}@", log: log, type: .info, String(describing: reason))
    ...
    override func wake() {
        ...
        let sharedDefaults = UserDefaults(suiteName: "group.com.craftnet.vpn")
        let bandwidthLimitMbps = sharedDefaults?.integer(forKey: "bandwidthLimitMbps") ?? 0
