...
class CraftNetVPNModule: RCTEventEmitter {
    ...
    override init() {
        ...
        print("[CraftNetVPN] Running in DEVELOPMENT MODE (simulator)")
        #else
    ...
    private func initializeLibrary() {
        // Initialize the CraftNet Rust library
        CraftNet.initialize()
        print("[CraftNetVPN] Rust library initialized via UniFFI")
    }
    ...
    private func loadVPNConfiguration() {
        NETunnelProviderManager.loadAllFromPreferences { [weak self] managers, error in
            ...
            self?.vpnManager = managers?.first { manager in
                ...
                return proto.providerBundleIdentifier == "com.craftnet.app.CraftNetVPN"
            }
        }
    }
    ...
    func connect(
        ...
        Task {
            do {
                ...
                let nodeConfig = CraftNet.config(
                    privacyLevel: privacyLevel,
                    bootstrapPeer: bootstrapPeer,
                    requestTimeoutSecs: 30
                )

                vpnClient = try CraftNetUnifiedNode(config: nodeConfig)
                try vpnClient?.start()
                ...
                resolve(nil)
            } catch let error as CraftNetError {
    ...
    func setCredits(
        ...
        // Store in user defaults for the extension to read
        UserDefaults(suiteName: "group.com.craftnet.vpn")?.set(creditValue, forKey: "credits")
        resolve(nil)
    }
    ...
    func request(
        ...
        if isDevelopmentMode {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                resolve([
                    "status": 200,
                    "body": "{\"mock\":true,\"method\":\"\(method)\",\"url\":\"\(urlString)\",\"headers\":\(headerCount),\"message\":\"Mock response from CraftNet\"}"
                ])
            }
            return
        }
    ...
    func selectExit(
        ...
        if isDevelopmentMode {
            print("[CraftNetVPN] selectExit: pubkey=\(pubkey)")
            resolve(nil)
            return
        }
    ...
    private func createVPNConfiguration(_ config: NSDictionary) async throws {
        ...
        proto.providerBundleIdentifier = "com.craftnet.app.CraftNetVPN"
        proto.serverAddress = "CraftNet P2P Network"
        ...
        manager.localizedDescription = "CraftNet VPN"
