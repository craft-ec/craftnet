...
/// React Native Native Module for TunnelCraft Daemon
/// Provides full daemon functionality for VPN and node operations
/// Uses Rust FFI bindings via uniffi
@objc(TunnelCraftDaemon)
class TunnelCraftDaemonModule: RCTEventEmitter {

    private var hasListeners = false

    // Rust FFI objects
    private var unifiedNode: TunnelCraftUnifiedNode?
    ...
    private func setupVPNManager() {
        ...
        vpnManager.onStatusChange = { [weak self] status in
            ...
            print("[TunnelCraftDaemon] VPN status changed: \(stateString)")
        ...
        vpnManager.loadConfiguration { [weak self] error in
            if let error = error {
                print("[TunnelCraftDaemon] Failed to load VPN config: \(error)")
            } else {
                print("[TunnelCraftDaemon] VPN configuration loaded")
    ...
    private func initializeRustBindings() {
        ...
        do {
            ...
            print("[TunnelCraftDaemon] Settlement configured successfully")
        } catch {
            print("[TunnelCraftDaemon] Failed to configure settlement: \(error)")
        }
        ...
        do {
            ...
            print("[TunnelCraftDaemon] Node pubkey set: \(pubkeyHex.prefix(16))...")
            ...
            print("[TunnelCraftDaemon] Credit hash set: \(creditHashHex.prefix(16))...")

            print("[TunnelCraftDaemon] Rust bindings initialized successfully")
        } catch {
            print("[TunnelCraftDaemon] Failed to initialize keys: \(error)")
        }
    }
    ...
    private func loadOrGenerateKeys() throws -> (privateKey: Data, publicKey: Data) {
        if keychainManager.hasPrivateKey() {
            ...
            print("[TunnelCraftDaemon] Loaded existing keys from keychain")
            ...
            print("[TunnelCraftDaemon] Generated and stored new keys")
            return (privateKey, publicKey)
        }
    }
    ...
    func connect(
        ...
        print("[TunnelCraftDaemon] connect() called with config: \(config)")

        // Save configuration to shared UserDefaults for Network Extension
        if let defaults = UserDefaults(suiteName: "group.com.tunnelcraft.vpn") {
        ...
        guard vpnConfigured else {
            print("[TunnelCraftDaemon] VPN not configured, loading configuration...")
            vpnManager.loadConfiguration { [weak self] error in
                if let error = error {
    ...
    private func startVPNTunnel(resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        ...
        if inFallbackMode {
            print("[TunnelCraftDaemon] Running in fallback mode (P2P only, no system VPN)")
        } else {
            print("[TunnelCraftDaemon] Starting VPN tunnel...")
        }
        ...
        vpnManager.startTunnel { [weak self] error in
            ...
            if let error = error {
                print("[TunnelCraftDaemon] Failed to start VPN: \(error)")
                reject("VPN_START_ERROR", error.localizedDescription, error)
                return
            }

            if inFallbackMode {
                print("[TunnelCraftDaemon] Fallback mode - starting P2P node for discovery")
            } else {
                print("[TunnelCraftDaemon] VPN tunnel start requested successfully")
            }
    ...
    private func startP2PNode() {
        ...
        guard unifiedNode == nil else {
            print("[TunnelCraftDaemon] P2P node already running")
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                let node = try TunnelCraftUnifiedNode(config: nodeConfig)
                try node.start()

                self.unifiedNode = node
                print("[TunnelCraftDaemon] P2P node started successfully")
                ...
                print("[TunnelCraftDaemon] Failed to start P2P node: \(error)")
    ...
    func disconnect(
        ...
        print("[TunnelCraftDaemon] disconnect() called")
        stopStatsUpdates()
        ...
        if let node = unifiedNode {
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                do {
                    ...
                    print("[TunnelCraftDaemon] Error stopping unified node: \(error)")
                }
            }
        }
    ...
    func setNodeMode(
        ...
        if let node = unifiedNode {
            do {
                ...
                print("[TunnelCraftDaemon] Failed to set mode: \(error)")
            }
        }
    ...
    func setBandwidthLimit(
        ...
        UserDefaults(suiteName: "group.com.tunnelcraft.vpn")?.set(mbps.intValue, forKey: "bandwidthLimitMbps")
    ...
    func setSplitTunnelRules(
        ...
        print("[TunnelCraftDaemon] Setting split tunnel rules: \(rulesArray.count) rules, mode: \(mode)")
    ...
    func getSplitTunnelRules(
        ...
        guard let defaults = UserDefaults(suiteName: "group.com.tunnelcraft.vpn") else {
            resolve([
                "mode": "exclude",
                "rules": []
            ])
    ...
    private func sendEpochUpdate() {
        ...
        do {
            ...
            print("[TunnelCraftDaemon] Failed to get epoch info: \(error)")
        }
    }
    ...
    private func sendPointsUpdate() {
        ...
        do {
            ...
            print("[TunnelCraftDaemon] Failed to get node points: \(error)")
        }
    }
    ...
    private func recordEpochEarnings() {
        ...
        do {
            ...
            print("[TunnelCraftDaemon] Failed to record epoch earnings: \(error)")
        }
    }
