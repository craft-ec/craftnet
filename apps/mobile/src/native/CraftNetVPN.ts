...
const {CraftNetVPN: NativeVPN} = NativeModules as {
  CraftNetVPN: CraftNetVPNModule;
};

if (!NativeVPN) {
  console.error(
    'CraftNetVPN native module not found. Make sure you have linked the native code.',
  );
}

// Event emitter for status updates
const vpnEventEmitter = NativeVPN
  ? new NativeEventEmitter(NativeModules.CraftNetVPN)
  : null;

// VPN API
export const CraftNetVPN = {
  /**
